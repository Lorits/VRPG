...
namespace Valve.VR.InteractionSystem
{
    ...
    public class LinearMoveDrawer : MonoBehaviour
    {
        ...
        protected virtual void HandAttachedUpdate(Hand hand)
        {
            ...
            // movelocked = false;
            if (!soundPlaying && moving) { soundscript.Play(); soundPlaying = true; }
            if (!movelocked)
            {
                UpdateLinearMapping(hand.transform);

                if (!moving && linearMapping.value < .95 && linearMapping.value > 0.05)
                {
                    moving = true;
                    soundscript.SetAudioSource(0);
                    soundscript.Play();
                }
                if ((linearMapping.value < 0.05 || linearMapping.value > .95) && moving)
                {
                    soundPlaying = false;
                    soundscript.Stop();
        ...
        protected void UpdateLinearMapping(Transform updateTransform)
        {
            ...
            linearMapping.value = prevMapping + Mathf.Clamp(linearMapping.value - prevMapping, -speedLimit, speedLimit);
