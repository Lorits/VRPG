...
namespace Valve.VR.InteractionSystem
{
    ...
    public class HingeInteractable : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            if (attachMode == AttachMode.Force)
            {
                for (int i = 0; i < holdingHands.Count; i++)
                {
                    ...
                    //vdisplacement[0] = 0.0f;
                    //targetPoint[0] = 0.0f;
                    holdingBodies[i].AddForceAtPosition(attachForce * vdisplacement, targetPoint, ForceMode.Acceleration);
                    Vector3 force = -attachForceDamper * holdingBodies[i].GetPointVelocity(targetPoint);
                    //force[0] = 0.0f;
                    holdingBodies[i].AddForceAtPosition(force, targetPoint, ForceMode.Acceleration);
                }
            }
        }
    }
