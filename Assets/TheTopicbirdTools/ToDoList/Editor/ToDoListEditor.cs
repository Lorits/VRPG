...
public class ToDoListEditor : Editor {
    ...
    //Active, To_fix, To_do, Optional, Default, Finished // default 0.7211 0.8513 1.0000
    Color getColor(ToDoList.TextObject.State state){
		Color c = new Color(0.7211f, 0.8513f, 1.0000f);
        if (state == ToDoList.TextObject.State.Active){
            c = new Color(0.4f, 0.8f, 1.0f);
        }
        else if (state == ToDoList.TextObject.State.Finished){
             c = new Color(0.7211f, 0.8513f, 1.0000f);
        }
        else if (state == ToDoList.TextObject.State.To_fix){
			c = new Color(1.0f, 0.6f, 0.1f);
		} else if (state == ToDoList.TextObject.State.To_do){
            c = new Color(1.0f, 0.9f, 0.0f);
        }
        else if (state == ToDoList.TextObject.State.Optional)
        {
            c = new Color(1.0f, 0.95f, 0.6f);
        }
        return c;
	}
